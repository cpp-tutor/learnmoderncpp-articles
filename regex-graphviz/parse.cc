...
PI_ const s_productionInfo[] = 
{
     ...
     {265, 3}, // 1: start (EOF_) ->  #0001 regex EOF_
     {267, 0}, // 2: #0001 ->  <empty>
     {266, 2}, // 3: regex (STAR) ->  regex STAR
     {266, 2}, // 4: regex (QUESTION) ->  regex QUESTION
     {266, 2}, // 5: regex (PLUS) ->  regex PLUS
     {266, 2}, // 6: regex ->  regex regex
     {266, 3}, // 7: regex (BAR) ->  regex BAR regex
     {266, 3}, // 8: regex (LPAREN) ->  LPAREN regex RPAREN
     {266, 1}, // 9: regex (CHAR) ->  CHAR
     {268, 1}, // 10: start_$ ->  start
};
...
#line 17 "grammar.y"
        {
         d_val_ = vs_(-1);
         nodes.at(d_val_.first - 1).addEdgeTo(nodes.size());
         nodes.at(d_val_.second).addEdgeTo(d_val_.first);
         }
        break;
...
#line 30 "grammar.y"
        { 
         ...
         nodes.at(vs_(-1).second).addEdgeTo(vs_(0).first);
         nodes.at(vs_(0).second).addEdgeTo(nodes.size());
         }
        break;
...
#line 35 "grammar.y"
        {
         d_val_ = { vs_(-2).first, nodes.size() };
         nodes.at(vs_(-2).second).addEdgeTo(d_val_.second);
         nodes.at(d_val_.first - 1).addEdgeTo(vs_(0).first);
         nodes.at(vs_(0).second).addEdgeTo(d_val_.second);
         nodes.emplace_back("", d_val_.second);
         nodes.at(d_val_.second).addEdgeTo(nodes.size());
         }
        break;
...
#line 46 "grammar.y"
        {
         d_val_ = { nodes.size(), nodes.size() };
         nodes.emplace_back(scanner.matched(), d_val_.first);
         }
        break;
